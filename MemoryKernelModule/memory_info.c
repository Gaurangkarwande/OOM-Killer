...
int MemoryInfoMonitor(void *data)
{
	while(!kthread_should_stop())
	{
		...
		if((memory_data.percentMemoryAvailable <= KILL_THRESHOLD_AVAILABLE_MEM) && (memory_data.percentFreeSwap <= KILL_THRESHOLD_SWAP_MEM))
		{
			OOM_Status = 2;
		}
		else if((memory_data.percentMemoryAvailable <= WARN_THRESHOLD_AVAILABLE_MEM) && (memory_data.percentFreeSwap <= WARN_THRESHOLD_SWAP_MEM))
		{
			OOM_Status = 1;
		}
		else
